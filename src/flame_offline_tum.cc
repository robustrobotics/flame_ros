 ...
 *
 * @file flame_offline_tum.cc
 * @author W. Nicholas Greene
 * @date 2016-09-18 02:04:03 (Sun)
 */
 ...
 * Input is taken from an input file using the TUM RGB-D benchmarks format:
 ...
 *
 *   timestamp tx ty tz qx qy qz qw rgb/frame.png [depth/frame.png]
 *
 * where timestamp is in seconds. The depth images are optional.
 */
class FlameOfflineTUM final {
 public:
  /**
   * \brief Constructor.
   */
  FlameOfflineTUM() :
    ...
    getParamOrFail(pnh_, "input_file", &input_file);

    std::string calib_file;
    getParamOrFail(pnh_, "calib_file", &calib_file);

    std::string input_frame_str;
    getParamOrFail(pnh_, "input_frame", &input_frame_str);
    ...
    getParamOrFail(pnh_, "depth_scale_factor", &depth_scale_factor);
  ...
  ~FlameOfflineTUM() = default;

  FlameOfflineTUM(const FlameOfflineTUM& rhs) = delete;
  FlameOfflineTUM& operator=(const FlameOfflineTUM& rhs) = delete;

  FlameOfflineTUM(FlameOfflineTUM&& rhs) = delete;
  FlameOfflineTUM& operator=(FlameOfflineTUM&& rhs) = delete;
  ...
  void main() {
    ...
    while (ros::ok() && !input_->empty()) {
...
#if 0
      bool corrupt_poses = false; // TODO(wng): REMEMBER TO TURN THIS OFF!
      if (corrupt_poses) {
        ROS_WARN("Artificially corrupting poses!\n");
        ROS_WARN("old_pose: q = %f, %f, %f, %f, t = %f, %f, %f\n",
        ...
        // For ICCV17 evaluation. Add artificial noise to poses.
        float rot_sigma = 0.01f * M_PI / 180.0f;
        float trans_sigma = 0.01f;

        static thread_local std::mt19937 rng(0);
        std::normal_distribution<float> rot_dist(0.0, rot_sigma);
        std::normal_distribution<float> trans_dist(0.0, trans_sigma);

        // First 3 elements are translation, last 3 are rotational.
        Sophus::SE3f::Tangent lie;
        lie(0) = trans_dist(rng);
        lie(1) = trans_dist(rng);
        lie(2) = trans_dist(rng);
        lie(3) = 0 * rot_dist(rng);
        lie(4) = 0 * rot_dist(rng);
        lie(5) = 0 * rot_dist(rng);
        Sophus::SE3f noise(Sophus::SE3f::exp(lie));

        ROS_WARN("noise: q = %f, %f, %f, %f, t = %f, %f, %f\n",
                 noise.unit_quaternion().w(),
                 noise.unit_quaternion().x(),
                 noise.unit_quaternion().y(),
                 noise.unit_quaternion().z(),
                 noise.translation()(0),
                 noise.translation()(1),
                 noise.translation()(2));
        ...
        ROS_WARN("new_pose: q = %f, %f, %f, %f, t = %f, %f, %f\n",
                 pose.unit_quaternion().w(),
                 pose.unit_quaternion().x(),
                 pose.unit_quaternion().y(),
                 pose.unit_quaternion().z(),
      ...
      if (num_imgs_ % subsample_factor_ == 0) {
        // Eat data.
        processFrame(img_id, time, pose, rgb, depth);
      }
      ...
      ROS_INFO_COND(!params_.debug_quiet,
                    "FlameOfflineTUM/main(%i/%u) = %4.1fms/%.1fHz (%.1fHz)\n",
  ...
  void processFrame(const uint32_t img_id, const double time,
    ...
    if (!update_success) {
      ...
      ROS_WARN("FlameOfflineTUM: Unsuccessful update.\n");
      return;
    }
...
#pragma omp parallel for collapse(2) num_threads(params_.omp_num_threads) schedule(dynamic, params_.omp_chunk_size) // NOLINT
    ...
    ROS_INFO_COND(!params_.debug_quiet,
                  "FlameOfflineTUM/publishing = %4.1fms\n",
    ...
    ROS_INFO_COND(!params_.debug_quiet,
                  "FlameOfflineTUM/debug_publishing = %4.1fms\n",
                  stats_.timings("debug_publishing"));

    stats_.tock("process_frame");

    ROS_INFO_COND(!params_.debug_quiet,
                  "FlameOfflineTUM/process_frame = %4.1fms\n",
 ...
 private:
  ...
  // Input stream object.
  std::shared_ptr<ros_sensor_streams::TUMRGBDOfflineStream> input_;
...
int main(int argc, char *argv[]) {
  ...
  flame_ros::FlameOfflineTUM node;
  node.main();
